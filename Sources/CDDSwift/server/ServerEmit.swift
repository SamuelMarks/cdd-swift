...
public func emitServer(document: OpenAPIDocument) -> String {
    ...
    output += "        let response = Response(status: .ok)\n"
    output += "        response.headers.contentType = .init(type: \"text\", subType: \"event-stream\")\n"
    output += "        // Send endpoint url for POST messages\n"
    output += "        let endpointEvent = \"event: endpoint\\ndata: /mcp/message\\n\\n\"\n"
    output += "        response.body = .init(string: endpointEvent)\n"
    output += "        return response\n"
    output += "    }\n\n"
    output += "    app.post(\"mcp\", \"message\") { req async throws -> Response in\n"
    ...
    output += "        guard let authHeader = req.headers.bearerAuthorization else {\n"
    output += "             throw Abort(.unauthorized)\n"
    output += "        }\n"
    output += "        let rpcRequest = try req.content.decode(JSONRPCRequest<AnyCodable>.self)\n"
    output += "        // Resolves incoming tool calls to backend route handlers (Dynamic API-to-Tool Proxy)\n"
    output += "        // TODO: route rpcRequest.method to generated API handlers using authHeader\n"
    output += "        let result = JSONRPCResponse(id: rpcRequest.id, result: AnyCodable([\"status\": \"success\"]))\n"
