...
The integration tests for this toolchain currently fail when running natively against the Stoplight Prism mock server (`./local-test.sh only-test` with `ONLY_TEST=cdd-swift`).

## Instructions
1. Run the test locally for this specific language using:
   `cd .. && ONLY_TEST=cdd-swift ./local-test.sh only-test`
...
4. Verify your changes by ensuring `cd .. && ONLY_TEST=cdd-swift ./local-test.sh only-test` passes successfully (including the coverage validation and chaos saboteur audits).
...
The Swift SDK fails during the Happy Path test suite with `NSURLErrorDomain Code=-1011` (HTTP 4xx response). Inspect the `uploadFile` and `updatePetWithForm` operations for correct multipart and url-encoded payloads.
